...
export const Context = ({ className, usage, ...props }: ContextProps) => {
  const used = (usage as any)?.totalTokens ?? 0;
  const max =
    (usage as any)?.context?.totalMax ??
    (usage as any)?.context?.combinedMax ??
    (usage as any)?.context?.inputMax;
          ...
          <div className="mt-1 space-y-1">
            {(usage as any)?.cachedInputTokens &&
              (usage as any).cachedInputTokens > 0 && (
                <InfoRow
                  costText={(usage as any)?.costUSD?.cacheReadUSD?.toString()}
                  label="Cache Hits"
                  tokens={(usage as any)?.cachedInputTokens}
                />
              )}
            <InfoRow
              costText={(usage as any)?.costUSD?.inputUSD?.toString()}
              label="Input"
              tokens={(usage as any)?.inputTokens}
            />
            <InfoRow
              costText={(usage as any)?.costUSD?.outputUSD?.toString()}
              label="Output"
              tokens={(usage as any)?.outputTokens}
            />
            <InfoRow
              costText={(usage as any)?.costUSD?.reasoningUSD?.toString()}
              label="Reasoning"
              tokens={
            ...
            />
            {(usage as any)?.costUSD?.totalUSD !== undefined && (
              <>
                ...
                <div className="flex items-center justify-between pt-1 text-xs">
                  ...
                  <div className="flex items-center gap-2 font-mono">
                    ...
                    <span>
                      {Number.isNaN(
                        Number.parseFloat(
                          (usage as any).costUSD.totalUSD.toString()
                        )
                      )
                        ? "—"
                        : `$${Number.parseFloat(
